(function () {
    ...
    function ehrClaimOtherController(ClaimEditService, UIService) {
        ...
        _this.updateClmOther = function () {
            _this.claim.AcceptAssignment = _this.acceptAssignment?"C":null;
            _this.claim.OutsideLab = _this.outsideLab;
            _this.claim.EmploymentRelated = _this.employmentRelated;
            _this.claim.AutoAccident = _this.autoAccident?"C":null;
            _this.claim.OtherAccident = _this.otherAccident?"C":null;
            _this.claim.OutsideLabCharges = _this.outsideLabCharges;
            _this.claim.Accident.State = _this.accidentState.abbr;
        }
        ...
        _this.formatDates = function () {
            if (_this.claim.Dates) {
                _this.claim.Dates = {
                    Id: _this.claim.Id,
                    OnsetOfCurrent: new Date(UIService.parseDate(_this.claim.Dates.OnsetOfCurrent)),
                    InitialTreatment: new Date(UIService.parseDate(_this.claim.Dates.InitialTreatment)),
                    LastSeen: new Date(UIService.parseDate(_this.claim.Dates.LastSeen)),
                    Acute: new Date(UIService.parseDate(_this.claim.Dates.Acute)),
                    LastMenstrual: new Date(UIService.parseDate(_this.claim.Dates.LastMenstrual)),
                    LastXRay: new Date(UIService.parseDate(_this.claim.Dates.LastXRay)),
                    HearingVision: new Date(UIService.parseDate(_this.claim.Dates.HearingVision)),
                    LastWorked: new Date(UIService.parseDate(_this.claim.Dates.LastWorked)),
                    ReturnToWork: new Date(UIService.parseDate(_this.claim.Dates.ReturnToWork)),
                    Admission: new Date(UIService.parseDate(_this.claim.Dates.Admission)),
                    Discharge: new Date(UIService.parseDate(_this.claim.Dates.Discharge)),
                    Assumed: new Date(UIService.parseDate(_this.claim.Dates.Assumed)),
                    Property: new Date(UIService.parseDate(_this.claim.Dates.Property)),
                    Repricer: new Date(UIService.parseDate(_this.claim.Dates.Repricer)),
                    Other: new Date(UIService.parseDate(_this.claim.Dates.Other)),
                    DisabilityStart: new Date(UIService.parseDate(_this.claim.Dates.DisabilityStart)),
                    DisabilityEnd: new Date(UIService.parseDate(_this.claim.Dates.DisabilityEnd))
                };
            }
        };
        ...
        _this.init = function () {
            ...
            _this.formatDates();
            _this.acceptAssignment = (_this.claim.AcceptAssignment != null) && (_this.claim.AcceptAssignment == "C");
            _this.outsideLab = (_this.claim.OutsideLab != null) && _this.claim.OutsideLab;
            _this.employmentRelated = (_this.claim.EmploymentRelated != null) && _this.claim.EmploymentRelated;
            _this.autoAccident = (_this.claim.AutoAccident != null) && (_this.claim.AutoAccident == "C");
            _this.otherAccident = (_this.claim.OtherAccident != null) && (_this.claim.OtherAccident == "C");
            _this.outsideLabCharges = _this.claim.OutsideLabCharges;

            _this.states = [
                { fips: 1, abbr: "AL", state: "Alabama" },
                { fips: 2, abbr: "AK", state: "Alaska" },
                { fips: 4, abbr: "AZ", state: "Arizona" },
                { fips: 5, abbr: "AR", state: "Arkansas" },
                { fips: 6, abbr: "CA", state: "California" },
                { fips: 8, abbr: "CO", state: "Colorado" },
                { fips: 9, abbr: "CT", state: "Connecticut" },
                { fips: 10, abbr: "DE", state: "Delaware" },
                { fips: 11, abbr: "DC", state: "District of Columbia" },
                { fips: 12, abbr: "FL", state: "Florida" },
                { fips: 13, abbr: "GA", state: "Georgia" },
                { fips: 15, abbr: "HI", state: "Hawaii" },
                { fips: 16, abbr: "ID", state: "Idaho" },
                { fips: 17, abbr: "IL", state: "Illinois" },
                { fips: 18, abbr: "IN", state: "Indiana" },
                { fips: 19, abbr: "IA", state: "Iowa" },
                { fips: 20, abbr: "KS", state: "Kansas" },
                { fips: 21, abbr: "KY", state: "Kentucky" },
                { fips: 22, abbr: "LA", state: "Louisiana" },
                { fips: 23, abbr: "ME", state: "Maine" },
                { fips: 24, abbr: "MD", state: "Maryland" },
                { fips: 25, abbr: "MA", state: "Massachusetts" },
                { fips: 26, abbr: "MI", state: "Michigan" },
                { fips: 27, abbr: "MN", state: "Minnesota" },
                { fips: 28, abbr: "MS", state: "Mississippi" },
                { fips: 29, abbr: "MO", state: "Missouri" },
                { fips: 30, abbr: "MT", state: "Montana" },
                { fips: 31, abbr: "NE", state: "Nebraska" },
                { fips: 32, abbr: "NV", state: "Nevada" },
                { fips: 33, abbr: "NH", state: "New Hampshire" },
                { fips: 34, abbr: "NJ", state: "New Jersey" },
                { fips: 35, abbr: "NM", state: "New Mexico" },
                { fips: 36, abbr: "NY", state: "New York" },
                { fips: 37, abbr: "NC", state: "North Carolina" },
                { fips: 38, abbr: "ND", state: "North Dakota" },
                { fips: 39, abbr: "OH", state: "Ohio" },
                { fips: 40, abbr: "OK", state: "Oklahoma" },
                { fips: 41, abbr: "OR", state: "Oregon" },
                { fips: 42, abbr: "PA", state: "Pennsylvania" },
                { fips: 72, abbr: "PR", state: "Puerto Rico" },
                { fips: 44, abbr: "RI", state: "Rhode Island" },
                { fips: 45, abbr: "SC", state: "South Carolina" },
                { fips: 46, abbr: "SD", state: "South Dakota" },
                { fips: 47, abbr: "TN", state: "Tennessee" },
                { fips: 48, abbr: "TX", state: "Texas" },
                { fips: 49, abbr: "UT", state: "Utah" },
                { fips: 50, abbr: "VT", state: "Vermont" },
                { fips: 51, abbr: "VA", state: "Virginia" },
                { fips: 78, abbr: "VI", state: "Virgin Islands" },
                { fips: 53, abbr: "WA", state: "Washington" },
                { fips: 54, abbr: "WV", state: "West Virginia" },
                { fips: 55, abbr: "WI", state: "Wisconsin" },
                { fips: 56, abbr: "WY", state: "Wyoming" }
            ];
            if (_this.claim.Accident == null) {
                _this.accidentState = _this.states[32];
                _this.claim.Accident = {
                    Id: _this.claim.Id,
                    Causes: ((_this.autoAccident ? "AA" : "") + (_this.otherAccident ? "OA" : "")),
                    State: _this.accidentState.abbr,
                    Country: "US"
                };
            } else {
                for (let i = 0; i < _this.states.length; i++) {
                    if (_this.claim.Accident.State == _this.states[i].abbr) {
                        _this.accidentState = _this.states[i];
                        break;
                    }
                }
            }
